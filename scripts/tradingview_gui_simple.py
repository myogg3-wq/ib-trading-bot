#!/usr/bin/env python3
"""
TradingView Alert Setup - Simple Windows GUI
Tkinter 기반 (Python 기본 라이브러리)
"""
...
try:
    ...
    print("필수 라이브러리 설치 중...")
    import subprocess
    subprocess.check_call([sys.executable, "-m", "pip", "install", "-q", "pydantic-settings"])
...
class TradingViewGUI:
    ...
    def create_watchlist_tab(self, notebook):
        ...
        # 입력 영역
        input_label = tk.Label(frame, text="종목 입력 (쉼표 또는 줄바꿈으로 구분):",
        ...
        # 종목 목록
        list_label = tk.Label(frame, text="🔍 인식된 종목:", font=("Arial", 10, "bold"), bg=self.bg_color)
    ...
    def create_manual_tab(self, notebook):
        ...
        # 종목 입력
        tk.Label(frame, text="종목 심볼:", font=("Arial", 10, "bold"), bg=self.bg_color).pack(anchor="w", padx=15, pady=10)
        ...
        # 동작 선택
        tk.Label(frame, text="동작 선택:", font=("Arial", 10, "bold"), bg=self.bg_color).pack(anchor="w", padx=15, pady=(15, 5))
        self.action_var = tk.StringVar(value="BUY & SELL")

        for action in ["BUY", "SELL", "BUY & SELL"]:
            tk.Radiobutton(
                frame, text=action, variable=self.action_var, value=action,
                font=("Arial", 10), bg=self.bg_color
            ).pack(anchor="w", padx=30)

        # 미리보기
        tk.Label(frame, text="📋 JSON 미리보기:", font=("Arial", 10, "bold"), bg=self.bg_color).pack(anchor="w", padx=15, pady=(15, 5))

        self.preview_text = scrolledtext.ScrolledText(frame, height=10, width=80, font=("Courier", 9))
    ...
    def create_result_tab(self, notebook):
        ...
        # 결과 텍스트
        self.result_text = scrolledtext.ScrolledText(frame, height=25, width=100, font=("Courier", 9))
    ...
    def create_settings_tab(self, notebook):
        ...
        # Webhook Secret
        tk.Label(frame, text="🔐 Webhook Secret:", font=("Arial", 10, "bold"), bg=self.bg_color).pack(anchor="w", padx=15, pady=(15, 5))
        ...
        # Webhook URL
        tk.Label(frame, text="🔗 Webhook URL:", font=("Arial", 10, "bold"), bg=self.bg_color).pack(anchor="w", padx=15, pady=(15, 5))
...
- 리모트 배포 시 https://your-domain.com/webhook 형식"""
        ...
        info_label = tk.Label(frame, text=info_text, font=("Arial", 9), bg="#FFF3CD", justify=tk.LEFT)
    ...
    def generate_alerts(self):
        ...
        messagebox.showinfo("완료", f"✅ {len(selected_tickers) * 2}개 알림 생성 완료!\n\n'결과' 탭에서 JSON을 확인하세요.\n각 JSON을 TradingView에 복붙해주세요.")
    ...
    def save_file(self):
        ...
        try:
            ...
            messagebox.showinfo("완료", f"✅ 저장됨: {file_path}")
